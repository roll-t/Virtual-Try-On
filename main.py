...
if __name__ == '__main__':
    ...
    # Tạo một kernel dùng cho phép biến đổi hình thái học với kích thước (3,3).
    k = cv2.getStructuringElement(cv2.MORPH_RECT, (3,3))
    # làm mờ đường biên 
    mask_img = cv2.erode(mask_img, k)
    # áp mặt nạ phân đoạn lên ảnh gốc
    img_seg=cv2.bitwise_and(ori_img,ori_img,mask=mask_img)
    # tính phần nền còn dư bằng hiệu ảnh gốc và ảnh phân đoạn
    back_ground=ori_img-img_seg
    # Đổi màu các pixel có giá trị 0 trong ảnh phân đoạn thành màu xám (215 là giá trị màu xám được sử dụng).
